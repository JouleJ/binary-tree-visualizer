...
namespace lib {
    ...
    namespace {
        ...
        public:
            ...
            bool validateValue(int64_t value) const override { return (value >= 0) && (value < static_cast<int64_t>(count)); }
        };
        ...
        public:
            explicit UpdateRequestScheme(size_t count): positionArgumentScheme(count), newValueArgumentScheme() {}
            ...
            const ArgumentScheme* getArgumentScheme(size_t idx) const override {
                switch (idx) {
                    case 0: return &positionArgumentScheme;
                    case 1: return &newValueArgumentScheme;
                    default: throw std::runtime_error("UpdateRequestScheme::getArgumentScheme: idx > 1");
                }
            }
        ...
        public:
            explicit GetSumRequestScheme(size_t count): leftBoundArgumentScheme(count), rightBoundArgumentScheme(count) {}

            QString getName() const override { return "Сумма на отрезке"; }

            bool postValidateValues(const int64_t* values) const override { return values[0] <= values[1]; }

            size_t getArgumentCount() const override { return 2; }

            const ArgumentScheme* getArgumentScheme(size_t idx) const override {
                switch (idx) {
                    case 0: return &leftBoundArgumentScheme;
                    case 1: return &rightBoundArgumentScheme;
                    default: throw std::runtime_error("GetSumRequestScheme::getArgumentScheme: idx > 1");
                }
            }
        ...
        class SegmentTree : public DataStructure {
            ...
            void update(size_t vertex_id, size_t left_bound, size_t right_bound, size_t position, int64_t new_value) {
                if (left_bound == right_bound) {
                ...
                if (position <= mid) {
                    ...
                    update(2 * vertex_id + 2, mid + 1, right_bound, position, new_value);
                }

                vertices[vertex_id] = vertices[2 * vertex_id + 1] + vertices[2 * vertex_id + 2];
            }

            int64_t getSum(size_t vertex_id, size_t left_bound, size_t right_bound, size_t req_left_bound, size_t req_right_bound) {
                ...
                if (req_right_bound <= mid) {
                    return getSum(2 * vertex_id + 1, left_bound, mid, req_left_bound, req_right_bound);
                }

                if (req_left_bound > mid) {
                    return getSum(2 * vertex_id + 2, mid + 1, right_bound, req_left_bound, req_right_bound);
                }

                return getSum(2 * vertex_id + 1, left_bound, mid, req_left_bound, mid) + getSum(2 * vertex_id + 2, mid + 1, right_bound, mid + 1, req_right_bound);
            }
        ...
        public:
            ...
            size_t getRequestSchemeCount() const override {
                return 2;
            }

            const RequestScheme* getRequestScheme(size_t idx) const override {
                switch (idx) {
                    case 0: return &updateRequestScheme;
                    case 1: return &getSumRequestScheme;
                    default: throw std::runtime_error("SegmentTree::getRequestScheme: idx > 1");
                }
            }

            int64_t executeRequest(const RequestScheme* requestScheme, const int64_t* firstValue) override {
                ...
                throw std::runtime_error("SegmentTree::executeRequest: bad requestScheme");
            }
        };    
    }
