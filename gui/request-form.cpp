...
RequestForm::RequestForm(QWidget* parent, const lib::RequestScheme* _requestScheme): QFrame(parent), requestScheme(_requestScheme) {
    ...
    for (size_t idx = 0; idx < argumentCount; ++idx) {
        argumentNameLabels[idx] = new QLabel(requestScheme->getArgumentScheme(idx)->getName(), this);
        argumentValueLineEdits[idx] = new QLineEdit(this);
        formLayout->addRow(argumentNameLabels[idx], argumentValueLineEdits[idx]);

        QObject::connect(argumentValueLineEdits[idx], &QLineEdit::textChanged, this, &RequestForm::onArgumentValueChanged);
    }
...
void RequestForm::onArgumentValueChanged(const QString& _ignored) {
    ...
    if (validateRequest(requestScheme, requestValues.data(), requestValues.size())) {
        isRequestValid = true;
    } else {
        requestValues.clear();
    }
...
void RequestForm::onAdjacentButtonClicked() {
    emit userRequested(requestScheme, isRequestValid, requestValues.data(), requestValues.size());
}

Requester::Requester(QWidget* parent, lib::DataStructure* _dataStructure): dataStructure(_dataStructure) {
    setFrameShape(QFrame::Box);
    formLayout = new QFormLayout;
    ...
    for (size_t idx = 0; idx < requestSchemeCount; ++idx) {
        const lib::RequestScheme* requestScheme = dataStructure->getRequestScheme(idx);
        ...
        QObject::connect(requestButtons[idx], &QPushButton::clicked, requestForms[idx], &RequestForm::onAdjacentButtonClicked);
        QObject::connect(requestForms[idx], &RequestForm::userRequested, this, &Requester::onUserRequested);
    }

    setLayout(formLayout);
}

void Requester::onUserRequested(const lib::RequestScheme* requestScheme, bool isRequestValid, const int64_t* firstValue, size_t valueCount) {
    if (isRequestValid) {
        ...
        const size_t requestResult = dataStructure->executeRequest(requestScheme, firstValue);
