...
DataStructureViewer::DataStructureViewer(QWidget *parent,
...
void DataStructureViewer::refreshNodes() {
    ...
    for (auto& row: rows) {
        ...
        if (nodeCount > 0) {
            ...
            const qreal xMax = row.nodes[nodeCount - 1].x + row.nodes[nodeCount - 1].width;
...
void DataStructureViewer::recurseTree(const lib::Node* node, size_t row_id) {
    ...
    out_node.width = 32. * out_node.text.length(); // TODO: Убрать магические числа
...
QSize DataStructureViewer::sizeHint() const {
